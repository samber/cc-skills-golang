...
Create a Github Action workflow with a skill installation step:
  ...
  - uses: anthropics/claude-code-action@v1
    with:
      prompt: |
        ...
        When reviewing Go code in this repository, apply the following guidelines. These instructions are authoritative — they override default behavior.

        ## Scope of Review

        **Do not limit the review to the lines changed in the pull request or commit.**

        A good review operates at two levels:

        1. **Diff-level review** — Examine every changed file and line directly. Flag bugs, style violations, security issues, and quality problems introduced or visible in the diff.
        2. **Higher-level review** — Step back and assess the broader impact of the change:
           - Does the change affect contracts or behavior that callers elsewhere in the codebase depend on?
           - Does it introduce architectural drift (e.g., breaks layering, adds circular dependencies, mixes concerns)?
           - Are there missing pieces — tests for the new code, documentation for new exports, error paths left unhandled?
           - Does it create or worsen existing technical debt patterns in the surrounding code?
           - Does the overall design make sense, or would a simpler approach achieve the same goal?

        When raising a higher-level concern, clearly state that it goes beyond the diff and explain the impact.

        ## Review Areas

        Perform independent reviews for each of the following areas. Apply the corresponding skill guidelines for each:

        - **Code style** — `/golang-code-style` skill
        - **Naming** — `/golang-naming` skill
        - **Error handling** — `/golang-error-handling` skill
        - **Concurrency** — `/golang-concurrency` skill
        - **Code safety** — `/golang-safety` skill
        - **Tests** — `/golang-testing` skill
        - **Performance** — `/golang-performance` skill
        - **Security** — `/golang-security` skill
        - **Dependencies** — `/golang-dependency-management` skill
        - **Documentation** — `/golang-documentation` skill
        - **Observability** — `/golang-observability` skill
        - **Modernize code** — `/golang-modernize` skill

        ## How to Report Issues

        For each issue found:
        1. **Classify the severity:**
           - **Blocking** — Must be fixed before merge (bugs, security vulnerabilities, data races, panics, broken contracts)
           - **Important** — Should be fixed soon but not a hard blocker (missing tests, poor error handling, misleading naming)
           - **Suggestion** — Style, readability, or quality improvement (optional but encouraged)
        2. **Anchor to a location** — Reference the file and line number for diff-level issues. For higher-level concerns, describe the affected area or pattern.
        3. **Explain the problem** — State why it is a problem, not just what it is.
        4. **Provide a fix** — Give a concrete suggestion or corrected code snippet when possible.

        Do not flag issues that are not present in the diff or the codebase as viewed. Do not invent violations.
...
Copy the skills in your repository using:
